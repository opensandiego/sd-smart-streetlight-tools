# GET_TFEVT_by_assetUid.py
# This function retrieves all the TRAFFIC events from a user-created list of sensors
...
def get_events(assetUid, label, dateList, startsArray, endtsArray, headers):
    ...
    for idx in range(len(startsArray)):
        ...
        url = "https://sandiego.cityiq.io/api/v2/event/assets/" + \
              assetUid + "/events?eventType=TFEVT&startTime=" + str(int(startsArray[idx])) + \
        ...
        try:
            ...
            oneLoc_df['direction_unit'] = [d.get('speedUnit') for d in obj_df.properties]
            oneLoc_df['eventUid'] = [d.get('eventUid') for d in obj_df.properties]
            oneLoc_df['vehicle_type'] = [d.get('vehicleType') for d in obj_df.properties]
            oneLoc_df['vehicle_count'] = [d.get('vehicleCount') for d in obj_df.measures]
            oneLoc_df['vehicle_speed'] = [d.get('speed') for d in obj_df.measures]
            oneLoc_df['vehicle_direction'] = [d.get('direction') for d in obj_df.measures]
            oneLoc_df['vehicle_counter_count'] = [d.get('counter_direction_vehicleCount') for d in obj_df.measures]
            oneLoc_df['vehicle_counter_speed'] = [d.get('counter_direction_speed') for d in obj_df.measures]
            oneLoc_df['vehicle_counter_direction'] = [d.get('direction') for d in obj_df.measures]
