# GET_BICYCLE_by_assetUid.py
# This function retrieves all the BICYCLE events from a user-created list of sensors
...
def get_events(assetUid, label, dateList, startsArray, endtsArray, headers):
    ...
    for idx in range(len(startsArray)):
        ...
        url = "https://sandiego.cityiq.io/api/v2/event/assets/" + \
              assetUid + "/events?eventType=BICYCLE&startTime=" + str(int(startsArray[idx])) + \
        ...
        try:
            ...
            oneLoc_df['bicycle_count'] = [d.get('bicycleCount') for d in obj_df.measures]
            oneLoc_df['bicycle_speed'] = [d.get('speed') for d in obj_df.measures]
            oneLoc_df['bicycle_direction'] = [d.get('direction') for d in obj_df.measures]
            oneLoc_df['bicycle_counter_count'] = [d.get('counter_direction_bicycleCount') for d in obj_df.measures]
            oneLoc_df['bicycle_counter_speed'] = [d.get('counter_direction_speed') for d in obj_df.measures]
            oneLoc_df['bicycle_counter_direction'] = [d.get('direction') for d in obj_df.measures]
